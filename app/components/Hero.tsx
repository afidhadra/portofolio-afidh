  ...
  return (
    <section className="flex flex-col items-center justify-center min-h-screen bg-gradient-to-br from-blue-50 via-white to-purple-50 dark:from-gray-900 dark:via-black dark:to-gray-800 relative overflow-hidden">
      <div className="relative z-10 text-center px-4 animate-fade-in">
        <h1 className="text-4xl sm:text-5xl md:text-6xl lg:text-7xl font-extrabold text-gray-900 dark:text-white mb-6 sm:mb-8 leading-tight">
          {t('hero.greeting')} <span className="text-blue-600 dark:text-blue-400">{t('hero.name')}</span>
        </h1>
        <p className="text-lg sm:text-xl md:text-2xl text-gray-700 dark:text-gray-400 mb-8 sm:mb-10 max-w-3xl mx-auto px-2 leading-relaxed">
          {t('hero.role')}
        </p>
        <div className="flex flex-col sm:flex-row gap-4 sm:gap-6 justify-center w-full max-w-md sm:max-w-none">
          <Button variant="primary" href={`/${resolvedLocale}/projects`} className="w-full sm:w-auto text-lg px-8 py-4">
            {t('hero.viewWork')}
          </Button>
          <Button variant="outline" href={`/${resolvedLocale}/contact`} className="w-full sm:w-auto text-lg px-8 py-4">
            {t('hero.getInTouch')}
          </Button>
        </div>
